...
LANGUAGE_TOKENS = {
    'en': 'en_XX', 'de': 'de_DE', 'ca': 'ca_XX', 'ar': 'ar_AR', 'el': 'el_EL', 'es': 'es_XX',
    'it': 'it_IT', 'ja': 'ja_XX', 'ko': 'ko_KR', 'hi': 'hi_IN', 'pt': 'pt_XX', 'eu': 'eu_EU',
    'ru': 'ru_RU', 'pl': 'pl_PL', 'zh': 'zh_CN', 'fr': 'fr_XX', 'vi': 'vi_VN', 'sv': 'sv_SE', "gl": "gl_GL"
}
...
class AMR3(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, filepath):
        ...
        for path in filepath:
            ...
            print(path)
            graphs = load(path)
            lang = path.split("/")[-1].split(".")[0].split("-")[-1]
            ...
            for graph in graphs:
                ...
                try:
                    amr = linearizer.linearize_reduced(graph)
                    # _, amr = linearizer.linearize_extreme(graph)
                except Exception as e:
                    print(f"Failed to linearize graph {id}: {str(e)}")
                    continue
                
                lang_snt = f"Generate the graph from the {LANGUAGE_TOKENS[lang]} sentence: {snt}"
                yield lang_id, {
                    "id": lang_id,
                    "snt": lang_snt,
                    "amr": amr,
                    "metadata": metadata,
                    "lang": LANGUAGE_TOKENS.get(lang, 'en_XX')
                }
